...
func TestUpdateSelectedColumns(t *testing.T) {
	err := db.Model(&entity.User{}).Where("id = ?", "1").Updates(map[string]interface{} {
		"middle_name": "",
		"last_name": "Morro",
	}).Error
	assert.Nil(t, err)

	err = db.Model(&entity.User{}).Where("id = ?", "1").Update("password", "diubahlagi").Error
	assert.Nil(t, err)

	err = db.Where("id = ?", "1").Updates(entity.User{
		Name: entity.Name{
			FirstName: "Bagus",
			LastName: "Wicaksono",
		},
